...
class LazyCoala_AdminPageFramework_Form_View___Script_RepeatableField extends LazyCoala_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateLazyCoala_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.LazyCoalaCodes-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addLazyCoala_AdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removeLazyCoala_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.LazyCoalaCodes-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.LazyCoalaCodes-field' )
            .filter( function() {                 
                return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addLazyCoala_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addLazyCoala_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.LazyCoalaCodes-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.LazyCoalaCodes-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.LazyCoalaCodes-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.LazyCoalaCodes-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.LazyCoalaCodes-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateLazyCoala_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.LazyCoalaCodes-field' ).addBack().trigger( 
            'LazyCoalaCodes_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.LazyCoalaCodes-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.LazyCoalaCodes-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.LazyCoalaCodes-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.LazyCoalaCodes-field,.LazyCoalaCodes-fields,.LazyCoalaCodes-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeLazyCoala_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.LazyCoalaCodes-field' );
        var nodeFieldsContainer = $( this ).closest( '.LazyCoalaCodes-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
